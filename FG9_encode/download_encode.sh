...
sort -k 1,1 -k2,2n ${feature}_feature+anno.tmp |  awk -F"\t" '{ $2 = int($2); $3 = int($3); print $1"\t"$2"\t"$3"\t"$4"\t"$5"\t"$6"\t"$7"\t"$8"\t"$9"\t"$10"\t"$11"\t"$12"\t"$13"\t"$14"\t"$15}' > ${feature}_feature+anno.sorted.bed
rm ${feature}_feature+anno.tmp; done
